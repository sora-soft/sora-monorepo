...
export type {CompilerOptions, Program, Symbol} from 'typescript';
...
function extend(target: any, ..._: any[]): any {
  ...
  const to = Object(target) as any;

  for (let index = 1; index < arguments.length; index++) {
      const nextSource = arguments[index] as any;
 ...
 */
// prettier-ignore
const validationKeywords: Record<string, boolean> = {
  multipleOf: true,               // 6.1.
  maximum: true,                  // 6.2.
 ...
 */
const annotationKeywords: Record<string, true | undefined> = {
  description: true,
  default: true,
  examples: true,
...
export class JsonSchemaGenerator {
   ...
   */
  private parseCommentsIntoDefinition(symbol: ts.Symbol, definition: Definition, otherAnnotations: Record<string, any>): void {
    if (!symbol) {
      return;
    }
  ...
  private getClassDefinition(clazzType: ts.Type, definition: Definition): Definition {
    ...
    if (modifierFlags & ts.ModifierFlags.Abstract && this.inheritingTypes[fullName]) {
      ...
      const propertyDefinitions = props.reduce((all: Record<string, any>, prop) => {
        const propertyName = prop.getName();
  ...
  getTypeDefinition(
    ...
    // Parse comments
    const otherAnnotations: Record<string, any> = {};
    ...
    if (this.recursiveTypeRef.get(fullTypeName) === definition && !forceNotRef) {
      ...
      if (this.reffedDefinitions[fullTypeName]) {
        const annotations = Object.entries(returnedDefinition).reduce((acc: Record<string, any>, [key, value]) => {
...
export function buildGenerator(
  ...
  for (const pref in args) {
    if (args.hasOwnProperty(pref)) {
      (settings as any)[pref] = (args as any)[pref];
    }
  }
